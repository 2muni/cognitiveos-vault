...
This is the Issue #78 evidence and execution plan for the disconnected,
default-off canonical-root control-plane foundation from PR #76. The active
...
This branch is a temporary documented stack on the unmerged PR #76 head
`0bc3055c9d5e8b0acc640f82fc2b741e29d9a1dc`. The dependency is necessary
because PR #76 contains the disconnected `cognitiveos.control_plane` contracts
that Issue #78 tests. It is not an integration, merge, or approval of PR #76.
PR #76 must be independently reviewed and merged (or this evidence rebased)
...
| No-follow containment | Walks each absolute root and target component using `openat`-style descriptor-relative `O_NOFOLLOW` operations and compares `fstat` to no-follow `stat(..., dir_fd=...)`. | Symlink or hard-link aliases, lexical `..`, component escapes, special types, and changed descriptor/path identities refuse. |
| Namespace and device/inode binding | Reads only the process mount-namespace handle, hashes it to an opaque test identifier, and binds actual temporary-root and allowed-root device/inode values. | Substituted namespace or device/inode evidence returns the existing denial-only control-plane reason. |
...
| Crash/torn-write and lock replacement | Supplies malformed partial JSON and a controlled replacement of the locked path in the temporary directory. | `replay_state_unavailable`, no state repair, no replay state creation after lock replacement. |
...
Before attaching the output to Issue #78, confirm that the focused suite did
...
the Issue #77 gap but would still require a fresh independent security review
...
worktree, attach exact output and platform facts to Issue #78, then request an
